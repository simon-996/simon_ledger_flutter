...
void main() {
  ...
  testWidgets('account profile dialog uses preview first layout', (
    ...
    expect(
      find.byKey(const ValueKey('profile-avatar-page-view')),
      findsOneWidget,
    );
    expect(
    ...
    await tester.tap(find.byKey(const ValueKey('profile-avatar-option-face')));
    await tester.pumpAndSettle();

    expect(
      find.descendant(
        of: find.byKey(const ValueKey('profile-dialog-avatar-preview')),
        matching: find.text('🙂'),
      ),
      findsOneWidget,
    );
