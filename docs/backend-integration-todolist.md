...
- [ ] 修复后端幂等实现的并发双执行窗口：当前先查幂等记录、执行业务后再保存记录，并发相同 `Idempotency-Key` 时可能重复执行写操作；应改为先占用幂等键或用数据库唯一键/状态机锁住请求，再执行业务并保存响应。
- [ ] 修复 Flutter 云端流水只读取第一页 100 条的问题：当前远端流水列表固定请求 `page=1&pageSize=100`，会导致超过 100 条后的账本详情、统计、筛选和分享结果不完整；应实现分页加载或改用后端统计接口。
