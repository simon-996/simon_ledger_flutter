...
- [x] 人员和流水 Provider 等待登录态恢复后再选择本地/远端仓储，避免登录后短暂读取本地空数据。
- [x] 账本下拉框选中态改为单行展示，弹出菜单保留名称和短标识，避免选中后溢出警告。
- [x] 远端流水编辑使用持久化的远端 UUID 和版本信息，避免 App 重启后编辑变成新增重复流水。
- [x] 远端流水删除支持本地待同步队列，没网时可先删除本地显示，联网后自动同步。
- [x] 记账列表展示待同步、同步失败状态，让用户知道离线记录是否已上传。
- [x] 云端创建账本时“加入本人”流程增加失败恢复和重试入口，避免账本创建成功但本人加入失败后无法处理。
- [x] 优化云端账本参与人加载方式，先将逐账本人员请求并发化，减少账本列表等待时间。
- [ ] 后端补充云端账本参与人批量查询接口，彻底减少账本列表 N+1 请求数量。
