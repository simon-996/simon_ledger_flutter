...
1. **应用包名尚未完成调整**：用户要求 Android/iOS 使用 `com.simon.ledger`，但当前 Android 仍为 `com.example.simon_ledger_flutter`，iOS 仍为 `com.example.simonLedgerFlutter`。发布前应统一修改并重新验证三端构建。
2. **离线同步仍需持续补测**：尤其验证本地临时账本同步成功后，再离线新增、编辑、删除人员和流水，随后恢复网络是否正确映射到远端账本。
3. **冲突处理仍偏基础**：流水依赖版本号避免覆盖，但 UI 还需要更明确地提示冲突并提供恢复动作。
4. **真实环境验收未全部勾选**：继续按 `docs/backend-acceptance-checklist.md` 在真实 API、数据库和多用户环境验证。
5. **本地存储容量有限**：当前使用 `shared_preferences` JSON。数据量增长后应评估迁移到三端兼容的结构化本地数据库。
6. **旧 Isar 数据不自动迁移**：如需要保留旧版本本地数据，应单独增加迁移入口。
