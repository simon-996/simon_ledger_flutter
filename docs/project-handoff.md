...
当前邀请加入链路已完成：API 的加入响应兼容旧版扁平邀请字段，并附带账本、成员和关联参与人快照；Flutter 会在加入响应返回后立即写入/恢复本地云账本和参与人，按 UUID 映射并保留待同步编辑。请求使用新的加入幂等键，并在写缓存前校验账号会话是否切换。邀请角色选择、成员管理 UI 和权威云端统计仍是后续切片。
