...
- 多端同时修改同一数据时进入冲突中心，由用户逐项选择本机或云端版本。
...
账户资料、账本、成员、参与人和流水都保存云端 `version`。更新、删除和恢复提交当前版本；API 返回 HTTP 409 / `409001` 时，客户端把安全的本机与云端快照保存到独立冲突队列，不把它计为普通同步失败，也不继续自动重试该实体。其他实体和其他账本仍会继续同步。

冲突入口位于首页提示、账本卡片的“冲突 N 项”和“我的 → 同步中心 → 数据冲突”。详情页默认只显示不同字段，并提供两个明确选择：

- 使用云端版本：直接应用已保存的云端快照，离线也可完成。
- 保留本机版本：以最新云端版本重新提交；离线时记录选择，联网后自动重试。

删除和恢复会再次确认具体动作。解决时若云端版本再次变化，原条目会更新为最新差异并要求重新确认。冲突记录使用 `local_store.conflicts.v1` 持久化，应用重启后仍可处理。
...
- 邀请码有过期时间、使用次数和默认角色。
- 加入账本必须登录。
- 加入成功会同时恢复/创建当前账号的账本成员和关联参与人；重复加入不会重复消耗次数。
- 客户端按远端 UUID 复用本地账本与参与人，保留待同步编辑；后续刷新失败时仍可立即记账和分摊。
- 邀请链接格式：
